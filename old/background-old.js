...
const audioCapture = (timeLimit, muteTab, format, quality, limitRemoved) => {
  ...
  chrome.tabCapture.capture({audio: true}, (stream) => { // sets up stream for capture
    ...
    // stream.addTrack();
    /*
    let deviceInfos = stream.getTracks();
    for (var i = 0; i < deviceInfos.length; ++i) {
      var deviceInfo = deviceInfos[i];
      alert("deviceInfo ___ " + deviceInfo.kind + " ___ " + deviceInfo.label + " ___ " + deviceInfo.deviceId );
    }

    stream.getTracks().forEach(function(track) {
      alert("track ___ " + track.kind + " ___ " + track.label + " ___ " + track.deviceId );
    });
    ...
    const stopCapture = function() {
      ...
      chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
        ...
        if(mediaRecorder && startTabId === endTabId){
          ...
          let completeCallbackNev = () => {
            chrome.tabs.sendMessage(tabs[0].id,{type: "sameSave", format: format, audio: audioURL, startID: startTabId});
          }
          setTimeout(completeCallbackNev, 1500);
